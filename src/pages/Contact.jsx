...
function TextInput({ name, placeholder, type = 'text', value, onChange, required }) {
  ...
  return (
    <MotionInput
      ...
      className="h-14 w-full rounded-full border border-steam-navy/20 bg-steam-navy/[0.02] px-6 text-[15px] text-steam-navy placeholder:text-steam-body/50 outline-none transition-all duration-200 hover:bg-steam-navy/[0.04] focus:bg-white focus:border-steam-green focus:ring-4 focus:ring-steam-green/10"
      animate={{
        boxShadow: focused
          ? '0 0 0 4px rgba(46,125,50,0.1), 0 2px 12px rgba(31,78,121,0.06)'
...
function TextAreaInput({ name, placeholder, value, onChange }) {
  ...
  return (
    <MotionTextarea
      ...
      onFocus={() => setFocused(true)}
      onBlur={() => setFocused(false)}
      className="min-h-[140px] w-full rounded-[24px] border border-steam-navy/20 bg-steam-navy/[0.02] p-5 px-6 text-[15px] text-steam-navy placeholder:text-steam-body/50 outline-none transition-all duration-200 hover:bg-steam-navy/[0.04] focus:bg-white focus:border-steam-green focus:ring-4 focus:ring-steam-green/10 resize-none"
      animate={{
        boxShadow: focused
          ? '0 0 0 4px rgba(46,125,50,0.1), 0 2px 12px rgba(31,78,121,0.06)'
          : '0 0 0 0px transparent',
  ...
  return (
    <div ref={ref} className="relative">
      ...
      <MotionButton
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex h-14 w-full items-center justify-between rounded-full border border-steam-navy/20 bg-steam-navy/[0.02] px-6 text-left outline-none transition-all duration-200 hover:bg-steam-navy/[0.04] focus:bg-white focus:border-steam-green focus:ring-4 focus:ring-steam-green/10 cursor-pointer"
        animate={{
          boxShadow: open
            ? '0 0 0 4px rgba(46,125,50,0.1), 0 2px 12px rgba(31,78,121,0.06)'
            : '0 0 0 0px transparent',
          borderColor: open ? 'rgb(46,125,50)' : 'rgba(31,78,121,0.2)',
        }}
        transition={{ duration: 0.2 }}
        aria-haspopup="listbox"
        aria-expanded={open}
      >
        <span className={value ? 'text-steam-navy text-[15px]' : 'text-steam-body/50 text-[15px]'}>
          {value || placeholder}
        </span>
      ...
      <AnimatePresence>
        {open && (
          <MotionUl
            ...
            transition={{ duration: 0.2 }}
            className="absolute left-0 right-0 top-[calc(100%+6px)] z-50 overflow-hidden rounded-[24px] border border-steam-navy/10 bg-white shadow-[0_12px_40px_rgba(31,78,121,0.12)]"
          >
            {options.map((opt, i) => (
              <MotionLi
                key={opt}
                ...
                className={[
                  'cursor-pointer px-6 py-3 text-[14px] font-medium transition-colors select-none',
                  value === opt
    ...
    <main className="section-wrap pb-14 pt-6">
      <Reveal className="panel-elevated p-6 shadow-brand md:rounded-3xl md:p-12">
        <div className="grid gap-10 md:grid-cols-[1fr_1.25fr]">
          ...
          {/* Right: form */}
          <form className="rounded-[28px] border border-steam-navy/10 bg-white p-6 shadow-[0_8px_40px_rgba(31,78,121,0.08)] md:p-10" onSubmit={handleSubmit}>
              ...
              {submitted ? (
                ...
                >
                  ...
                  <MotionButton
                    type="submit"
                    disabled={loading}
                    className="btn-cta mt-4 h-14 w-full rounded-full text-[15px] cursor-pointer disabled:opacity-60"
                    initial={{ opacity: 0, y: 8 }}
                    animate={{ opacity: 1, y: 0 }}
